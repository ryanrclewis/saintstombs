...
const generate = async () => {
  ...
  // Copy region markdown files to public/regions/ so they can be fetched at runtime
  const regionsDir = path.join(contentDir, 'regions')
  const publicRegionsDir = path.join(projectRoot, 'public', 'regions')
  await fs.mkdir(publicRegionsDir, { recursive: true })
  const regionFiles = await fs.readdir(regionsDir)
  await Promise.all(
    regionFiles
      .filter((f) => f.endsWith('.md'))
      .map((f) => fs.copyFile(path.join(regionsDir, f), path.join(publicRegionsDir, f)))
  )
}
